...
%% MC SIMULATION
state_paths_5y = MCpath(steps_per_year, total_years, num_simulations, M_switch); 
state_paths_1y = MCpath(steps_per_year, 1, num_simulations, M_switch); 

%% Calculate the 5-year and 1-year Transition Matrices -> 3d matrix
transition_matrices5y = zeros(num_ratings, num_ratings, num_simulations); % I need a 3d matrix -> 3rd dim the simulations

% Compute the Transition Matrices for each Simulation
for sim = 1:num_simulations
    transition_matrices5y(:, :, sim) = computeTransitionMatrix(state_paths_5y(sim, :), num_ratings, Me, Mc);
end
